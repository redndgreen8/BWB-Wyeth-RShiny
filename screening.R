...
df.phoneConsult <- getPhoneConsult(screen_str)
#PC <- getPieChartCombined(df.phoneConsult, "Dispositionflag", "dispo.png" , "Title" )
#PC
...
getPCFpie <- function(df.phoneConsult){
  ...
  ggsave("plots/PCF.png", gpPCF, height = 9, width = 16, dpi = 600)
...
getPCFpieComb <- function(df.phoneConsult){
  ...
  gpFlag <- ggplot(df.phoneConsultFlag, aes(x = "", y = pct, fill = fct_inorder(Dispositionflag))) +
    ...
    labs(subtitle = "This includes participants from Web and Clinic.",
         fill = "Race") 
  ggsave("plots/PCFC.png", gpFlag, height = 9, width = 16, dpi = 600)
...
getPCRconsiderpie <- function(df.phoneConsult){
  dfPCR <- df.phoneConsult %>%
    dplyr::filter(Dispositionflag =="Considering") %>%
  ...
  gpPCR <- ggplot(dfPCR, aes(x = "" , y = pct, fill = Race)) +
    ...
    theme(axis.text = element_blank(),
          ...
          plot.subtitle = element_text(hjust = 0.5)) +
    guides(fill = guide_legend(ncol = 2)) +
    ggtitle("Race Breakdown Based on location of approach (Considering)")  
  ggsave("plots/PCRconsider.png", gpPCR, height = 9, width = 16, dpi = 600)
  
  return(gpPCR)
  
}

consider_pie <- getPCRconsiderpie(df.phoneConsult)
#consider_pie


getPCRconsiderCOMBpie <- function(df.phoneConsult){
  
  df.phoneConsultRace <- df.phoneConsult |>
    select(-ID) |>
    filter(Dispositionflag == "Considering") |>
    group_by(Race) |>
    summarize(n = n()) |> 
    ungroup() |> 
    mutate(pct = round(n/sum(n), 3))
  ...
  gpRace <- ggplot(df.phoneConsultRace, aes(x = "", y = pct, fill = fct_inorder(Race))) +
    ...
    theme(axis.text = element_blank(),
          ...
          legend.position = "none") +
    guides(fill = guide_legend(ncol = 2)) +
    ggtitle("Race Demographics of those 'Considering'") + 
  ...
  #gpRace  
  ggsave("plots/PCRCconsider.png", gpRace, height = 9, width = 16, dpi = 600)
...
consider_pieC <- getPCRconsiderCOMBpie(df.phoneConsult)
consider_pieC

getPCRrefuseCOMBpie <- function(df.phoneConsult){
  
  df.phoneConsultRace <- df.phoneConsult |>
    select(-ID) |>
    filter(Dispositionflag == "Refused") |>
    group_by(Race) |>
    summarize(n = n()) |> 
    ungroup() |> 
    mutate(pct = round(n/sum(n), 3))
  ...
  gpRace <- ggplot(df.phoneConsultRace, aes(x = "", y = pct, fill = fct_inorder(Race))) +
    ...
    theme(axis.text = element_blank(),
          ...
          legend.position = "none") +
    guides(fill = guide_legend(ncol = 2)) +
    ggtitle("Race Demographics of those 'Refused'") + 
    labs(subtitle = "This includes participants from the Web and Clinics.",
         fill = "Race") 
  #ggsave("plots/Race_demog.png", gp, height = 9, width = 16, dpi = 600)
  #gpRace  
  ggsave("plots/PCRCrefuse.png", gpRace, height = 9, width = 16, dpi = 600)
...
refuse_pieC <- getPCRrefuseCOMBpie(df.phoneConsult)
refuse_pieC

getPCRrefusepie <- function(df.phoneConsult){
  dfPCR <- df.phoneConsult %>%
    dplyr::filter(Dispositionflag =="Refused") %>%
    dplyr::count(location, Race) %>%
    dplyr::group_by(location) %>%
    dplyr::mutate(total = sum(n)) %>%
    dplyr::mutate(pct = n / total) %>%
  ...
  gpPCR <- ggplot(dfPCR, aes(x = "" , y = pct, fill = Race)) +
    ...
    theme(axis.text = element_blank(),
          ...
          plot.subtitle = element_text(hjust = 0.5)) +
    guides(fill = guide_legend(ncol = 2)) +
    ggtitle("Race Breakdown Based on location of approach (Refused)")  
  ggsave("plots/PCRrefuse.png", gpPCR, height = 9, width = 16, dpi = 600)
  
  return(gpPCR)
  
}

#refuse_pie <- getPCRrefusepie(df.phoneConsult)
#refuse_pie
